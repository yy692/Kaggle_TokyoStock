...
def MovingAverage(xTr):
    ...
    std30 = np.zeros(len(xTr[0]))
    std60 = np.zeros(len(xTr[0]))
    
    ema5lst, ema10lst, ema20lst = [], [], []
    std30lst, std60lst = [], [] 
    for idx,x in enumerate(xTr):
        ...
        std30 = np.std(xTr[max(0,idx-30):idx+1], axis=0)
        std60 = np.std(xTr[max(0,idx-60):idx+1], axis=0)
        
        std30lst.append(std30.copy())
        std60lst.append(std60.copy())

    return np.array(ema5lst), np.array(ema10lst), np.array(ema20lst), np.array(std30lst), np.array(std60lst)
...
def mlp_model(input_dims, output_dims, n_layers, first_layer_nodes, last_layer_nodes, activation_func, loss_func):
    ...
    model.add(Dense(output_dims, kernel_regularizer=L2(0.1)))
